...
We strongly recommend you use all the prefixes (`-o-`, `-webkit-`, `-moz-`) when you provide gradients as image values (`background` or `border-image`). Do provide a fallback color if you are using this as a value for `background` so browsers that do not support the gradients can render a solid background color. Note, the `-ms-` prefix was only used by alpha versions of IE10.
